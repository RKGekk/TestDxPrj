...
void ObjLoader::LoadObjFile(std::string fileName) {
	...
	if(in.is_open()) {
		...
		while(std::getline(in, line)) {
			...
			if((keyword == "g" || keyword == "o") && tokensCount > 1) {
				if(!firstGroup) {
		...
		while(std::getline(in, line)) {
			...
			if(keyword == "newmtl" && tokensCount > 1) {
				if(mtlCount != 0) {
					...
					newMaterial.AmbientColor = XMFLOAT3(0.0f, 0.0f, 0.0f);
					newMaterial.DiffuseColor = XMFLOAT3(0.0f, 0.0f, 0.0f);
					newMaterial.SpecularColor = XMFLOAT3(0.0f, 0.0f, 0.0f);
					newMaterial.SpecularCoefficient = 0.0f;
					newMaterial.OpticalDensity = 0.0f;
					newMaterial.Transparency = 0.0f;
					newMaterial.IlluminationModel = 0;
					newMaterial.AmbientTextureMap = "";
					newMaterial.DiffuseTextureMap = "";
					newMaterial.SpecularTextureMap = "";
					newMaterial.SpecularHighlightTextureMap = "";
					newMaterial.AlphaTextureMap = "";
					newMaterial.BumpMap = "";
				}
